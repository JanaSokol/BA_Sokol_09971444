...
CONFIGDIRGFS=$WRFROOT/config/GFS
CONFIGDIRICON=$WRFROOT/config/ICON
...
cp $CONFIGDIRGFS/namelist.ARWpost $POSTDIR/namelist.ARWpost
...
cp $CONFIGDIRICON/namelist.ARWpost $POSTDIR/namelist.ARWpost
sed -i "s/STARTYEAR/$STARTYEAR/g" $POSTDIR/namelist.ARWpost
sed -i "s/STARTDAY/$sday/g" $POSTDIR/namelist.ARWpost
sed -i "s/STARTMONTH/$smon/g" $POSTDIR/namelist.ARWpost
sed -i "s/STARTHOUR/$shour/g" $POSTDIR/namelist.ARWpost
...
cp $WRFROOT/scripts/visualizeWithMarkers.gs $WRFROOT/GrADs/Contents/Resources/Scripts/visualizeWithMarkers.gs
sed -i "s/AMOUNT_OF_FILES/$AMOUNT_OF_FILES/g" $WRFROOT/GrADs/Contents/Resources/Scripts/visualizeWithMarkers.gs
rm $WRFROOT/GrADs/udpt
cp $WRFROOT/config/udpt $WRFROOT/GrADs/udpt

filenameGFS="GFS_arwpost_${STARTYEAR}-${smon}-${sday}_${shour}-${ENDYEAR}-${emon}-${eday}_${ehour}"
filenameICON="ICON_arwpost_${STARTYEAR}-${smon}-${sday}_${shour}-${ENDYEAR}-${emon}-${eday}_${ehour}"
time_step=${4}

grads -bpcx 'visualizeWithMarkers.gs '$filenameGFS' GFS '$time_step
grads -bpcx 'visualizeWithMarkers.gs '$filenameICON' ICON '$time_step


TARGETDIR=$WRFDIR/run/ANALYSIS
mv -v *.png $TARGETDIR
